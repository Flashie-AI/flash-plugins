...
The deterministic bash logic (pre-flight checks, clone, auto-detect, file generation, placeholder substitution) lives in `lib/setup-core.sh` inside this plugin. Same code runs in production and in tests. Source it at the top of any bash block:
...
2. Partial / corrupted setup (any of 6 files missing) → recovery command + exit 0.
...
### Step P2.1: Auto-detect from git email

```bash
EMAIL=$(git config user.email 2>/dev/null || true)
if fv_autodetect "$EMAIL"; then
  AUTO_DETECTED=1
else
  AUTO_DETECTED=0
fi
...
After `fv_autodetect` returns, these env vars are populated (or empty):
- `FV_PREFILL_NAME`, `FV_PREFILL_ROLE`, `FV_PREFILL_SQUAD`, `FV_PREFILL_LINES`, `FV_MATCH_FILE`

If a unique match was found and all four fields extracted, skip the open-ended Turn 1 and use the auto-detect confirmation in Part 3.1. Partial extraction or no match → standard Turn 1 in Part 2.

### Part 1: Welcome + Turn 1

**Auto-detect path** (full match — all four pre-fills present):

Print:

> Vault cloned to `$VAULT`. I matched your git email to `$FV_MATCH_FILE` and pulled your role + squad → product lines from there. Setting up your personal space now.

Then jump directly to Part 3.1 confirmation.

**Standard path** (no match, or partial match):
...
>
> First — tell me about yourself. What do you do at Flash, which product line(s) do you spend most of your time in, and what are you focused on right now?
...
After the Turn 1 response, extract these signals from the user's message. Apply pre-fills from auto-detect wherever the user did not override.
...
|---|---|---|
| Name | "I'm X" / "my name is X" / context | Required — never default. If absent AND no pre-fill: ask in Part 3.3. |
...
#### 3.1 Auto-detect confirmation

> From `$FV_MATCH_FILE`:
>
> - **You:** $FV_PREFILL_NAME, $FV_PREFILL_ROLE
> - **Squad:** $FV_PREFILL_SQUAD
> - **Product lines (from squad):** $FV_PREFILL_LINES
>
> Quick gut check: anything in the people record out of date? And what are you focused on right now? (Seeds the "What I'm focused on" section of your `personal/identity.md`.) If all good, paste your current focus.

After Turn 2 response: parse focus → set `FV_FOCUS`. If the user corrected anything (e.g. "I'm on growth too now"), update derived state and re-validate `FV_LINES` with `fv_validate_lines`. Then proceed to Part 4.

#### 3.2 Standard confirmation
...
>
> If any product lines are wrong, or you also work on others, tell me. Otherwise: ready to generate?

#### 3.3 Missing-required-field turns
...
After each answer, re-extract and re-validate. Only emit 3.2 once all three required fields are populated.

#### 3.4 Impatience handling — never default required fields

If the user says "ready", "go ahead", "just do it", or similar while any of `FV_NAME`, `FV_ROLE`, or `FV_LINES` is missing or invalid, do NOT substitute defaults. Acknowledge the urgency and re-ask only the missing field(s) using 3.3 prompts. Setup is blocked until all three are answered explicitly. The remaining fields (focus, personality) may be left empty if the user wants to skip them.

### Part 4: Generate (deterministic — no model decisions)

```bash
export FV_NAME FV_ROLE FV_LINES FV_PRIMARY_LINE FV_FOCUS FV_PERSONALITY
fv_generate
```
